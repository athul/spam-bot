...
def clean_res(message, re: bool = True) -> str:
    if "@" in message:
        ...
        if username != "spam_re_mon_bot":
            return (
                f"@{username} kutta, aa post Re aarunnu ketto".replace(
                    "_", "\_")
                if re
                else username.replace("_", "\_")
            )
        else:
...
def handle_one_liner(upd: Update, _: CallbackContext, message: str):
    ...
    -----
    message: str
        Takes in the message string from the TOML file specified within one_liners
    """
    try:
...
def handle_sticker_only_messages(upd: Update, _: CallbackContext, sticker: str):
    """
    Handles sticker only messages.

    Params
    -----
    sticker: str
        Takes in the path to where the sticker is stored. The path will be specified in the TOML file
    """
    try:
        upd.message.reply_sticker(
            open(sticker, "rb").read(),
            reply_to_message_id=upd.message.reply_to_message.message_id,
        )
...
def handle_reply_only_onliner(upd: Update, _: CallbackContext, message: str):
    """
    Handles reply only commands

    Params
    -----
    message: str
        Takes in the Message string from TOML file
    """
    try:
...
def handle_messages_with_stickers(
    upd: Update, _: CallbackContext, message: str, sticker: str
):
    """
    Handles commands which sends stickers and messages.
    ...
    message: str
        Takes in the Message string from TOML file
    sticker: str
        Takes in the path to sticker from TOML file
    """
    try:
        upd.message.reply_text(message)
        upd.message.reply_sticker(open(sticker, "rb").read())
...
def respond_with_frande(upd: Update, _: CallbackContext):
    """
    Checks for the specified strings in the `words` list
    If present sends a sticker
    """
    ...
    if not is_bot and any(x in txt for x in words):
        try:
            upd.message.reply_sticker(
                open("stickers/frand.webp", "rb").read(),
                reply_to_message_id=upd.message.reply_to_message.message_id,
            )
        except:
...
def handle_help(upd: Update, _: CallbackContext, message: str):
    """
    Handles the Help text

    Params
    ------
    message:str
        Takes in the help text defined in the TOML file.
    """
    upd.message.reply_markdown_v2(
        message.replace("-", "\-").replace("/", "\/").replace(".", "\.")
    )
...
def get_dispatcher():
    ...
    for key, value in data["messages"]["one_liners"].items():
        dp.add_handler(CommandHandler(
            key, __(handle_one_liner, message=value)))

    for key, value in data["messages"]["reply_only_one_liners"].items():
        dp.add_handler(
            CommandHandler(key, __(handle_reply_only_onliner, message=value))
        )

    for key, value in data["messages"]["stickers"].items():
        dp.add_handler(
            CommandHandler(
                key, __(handle_sticker_only_messages, sticker=value))
        )
    ...
    for key, value in data["messages"]["messages_with_stickers"].items():
        dp.add_handler(
            CommandHandler(
                key,
                __(handle_messages_with_stickers,
    ...
    dp.add_handler(
        CommandHandler("help", __(
            handle_help, message=data["constant"]["help"]))
    )
